...
(function ($) {
    ...
    function templateResultCustom(data, container) {
        if (data.element) {
            ...
            var $wrapper = $("<span>" + data.text + "</span><span></span>");
            var $switchSpn = $wrapper.last();
            ...
            if (hasParent($element)) {
                //tpo
                var _level = getTreeLevel($select, $element.val());
                var paddingLeft = _level * 2;
                $container.attr("data-level", _level);
                $container.attr("data-level", _level);
               //tpo
    ...
    function templateSelectionCustom(item) {
        ...
        if (parentsText != '') parentsText += ' &rArr; ';//selected value------------
